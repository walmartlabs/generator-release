...
module.exports = {
  originName: function() {
    var cb = this.async(),
        self = this;
    childProcess.exec('git remote show -n origin', function(err, stdout) {
      if (err) {
        throw new Error('originName: ' + err.message);
      }

      if (/Fetch URL: .*@([^:\/]*)[:\/]?(.*)\.git/.exec(stdout)) {
        self.host = RegExp.$1;
        self.repoName = RegExp.$2;
      } else if (/Fetch URL: https?:\/\/([^\/]+)\/(.*)/.exec(stdout)) {
        // Handle subversion-style urls
        self.host = RegExp.$1;
        self.repoName = RegExp.$2;
      } else {
        throw new Error('Unknown fetch url: ' + stdout);
      }
      cb();
    });
  },
  findFirstCommit: function() {
    ...
    var cb = this.async(),
        self = this;
    childProcess.exec('git rev-list HEAD --max-parents=0 --abbrev-commit', function(err, stdout) {
      if (err) {
  ...
  commitTime: function() {
    var cb = this.async(),
        self = this;
    childProcess.exec('git log -n 1 ' + this.firstCommit, function(err, stdout) {
      if (err && /unknown revision or path not in the working tree/.test(err.message)) {
        throw new Error('Tag "' + self.firstCommit + '" does not exist. Please check that this release tag exists and is available locally.');
      }
  ...
  findChanges: function() {
    ...
    childProcess.exec('git log --oneline ' + this.firstCommit + '...' + (this.lastCommit || 'HEAD'), function(err, stdout) {
      if (err) {
      ...
      repo.listIssues({state: 'closed', since: self.firstCommitTime}, function(err, issuesQuery) {
        ...
        async.each(issuesQuery, function(issue, callback) {
          ...
          function(err) {
            ...
            self.commits = _.reject(commits, isMergeCommit);
  ...
  pingPullRequests: function() {
    ...
    async.forEachSeries(this.pulls, function(pull, cb) {
        ...
        issue.comment('Released in ' + self.version, function(err) {
          if (err) {
            console.error(err);
            throw new Error(': ' + err.message);
          }
  ...
  ensureFetched: function() {
    var cb = this.async();
    childProcess.exec('git fetch', function(err) {
      if (err) {
        throw new Error('ensureFetched.fetch: ' + err.message);
      }

      childProcess.exec('git branch -v --no-color | grep -e "^\\*"', function(err, stdout) {
        if (err) {
...
function isMergeCommit(commit) {
  return (commit.title.indexOf('Merge pull request') === 0 ||
    commit.title.indexOf('Merge branch') === 0);
}
