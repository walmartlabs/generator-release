...
module.exports = {
            ...
            if (!issue.pull_request || !issue.pull_request.html_url) {
              ...
              github.getIssue(nameInfo[0], nameInfo[1], issue.number).events(function(err, data) {
                ...
                if (commitEvents.length) {
                  if (listHasSha(commits, commitEvents[0].commit_id)) {
  ...
  pingPullRequests: function() {
    var github = new Github(require(getUserHome() + '/.config/generator-release')),
        self = this,
        cb = this.async();
    ...
    async.forEachSeries(this.pulls, function(pull, cb) {
        // Use base from full pull requests and the issue otherwise
        var base = pull.base || pull,
