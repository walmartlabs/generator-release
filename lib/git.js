...
module.exports = {
  ...
  findChanges: function() {
    ...
    childProcess.exec('git log --max-parents=1 --oneline ' + this.firstCommit + '...' + (this.lastCommit || 'HEAD'), function(err, stdout, stderr) {
      if (err) {
      ...
      repo.listIssues({state: 'closed', since: self.firstCommitTime}, function(err, issuesQuery) {
        ...
        async.eachSeries(issuesQuery, function(issue, callback) {
...
function fetchGithub(url, callback) {
  ...
  request(options, function (error, response, body) {
    if (error) {
      ...
      throw new Error('fetchGithub: ' + error.message);
    }
...
function extractLinks(text, links) {
  ...
  return _.sortBy(_.union(links, foundLinks), 'title');
}
